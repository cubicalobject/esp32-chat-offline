...
void drawBootScreen(const String& ip) {
  ...
  do {
    ...
    u8f.setCursor(10, 140); u8f.print(ip.c_str());
    u8f.setFont(u8g2_font_6x13_t_cyrillic);
    u8f.setCursor(2, 165);  u8f.print("Waiting for users...");
  } while (display.nextPage());
}
...
void onWsEvent(AsyncWebSocket* s, AsyncWebSocketClient* c,
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (strcmp(mtype, "register") == 0) {
      ...
      if (nameOnline(name)) {
        ws.text(c->id(), "{\"type\":\"error\",\"text\":\"Already logged in elsewhere.\"}");
        return;
      }
      ...
      ws.text(c->id(), "{\"type\":\"loggedin\",\"name\":\"" + name + "\",\"new\":true}");
      sendHistory(c->id());
    ...
    // ── Login to existing account ─────────────────────────
      ...
      if (nameOnline(name)) {
        ws.text(c->id(), "{\"type\":\"error\",\"text\":\"Already logged in on another device.\"}");
        return;
      }
      if (!loginUser(c->id(), name)) {
        ws.text(c->id(), "{\"type\":\"error\",\"text\":\"Chat room is full (max 8).\"}");
...
<script>
let ws, myName = '', registered = false;
...
function handle(d) {
  if (d.type === 'ready') {
    // Server is ready — show auth screen
    showAuth();
    nameInput.focus();
    ...
    showChat();
    if (d.new) addSystem('Account created! Welcome, ' + d.name + '.');
    else        addSystem('Welcome back, ' + d.name + '! \u2713');
    msgInput.focus();
...
function logout() {
  ...
  showAuth();
  nameInput.focus();
  // Close and reopen WebSocket so server sees disconnect → logout
  ws.close();
}
...
<div style="text-align:center;">
  <p style="font-size:18px;margin-bottom:8px;">ESP32 Chat</p>
  <p style="font-size:13px;color:#555;">Opening chat...</p>
  <p style="margin-top:16px;font-size:12px;color:#333;">
    If it doesn't open automatically,<br>go to
    <span style="color:#7c6af7;">http://192.168.4.1</span>
  </p>
</div></body></html>
)rawliteral";
...
void setup() {
  ...
  server.on("/library/test/success.html", HTTP_GET, captive);
  server.on("/generate_204",              HTTP_GET, captive);
  server.on("/gen_204",                   HTTP_GET, captive);
  server.on("/connecttest.txt",           HTTP_GET, captive);
  server.on("/ncsi.txt",                  HTTP_GET, captive);
  server.on("/redirect",                  HTTP_GET, captive);
  server.on("/canonical.html",            HTTP_GET, captive);
